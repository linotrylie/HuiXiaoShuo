...
$.xget = function(url, callback, retry) {
    ...
    $.ajax({
        ...
        beforeSend:function (request) {
            var bear = $.cookie('access_token')
            request.setRequestHeader('Authorization','Bearer '+ bear)
        },
        xhrFields: {withCredentials: true},
        success: function(r){
            if(!r) return callback(-100,null, 'Server Response Empty!');
            var s = xn.json_decode(r);
            if(!s) {
                return callback(-101,null, r); // 'Server Response xn.json_decode() failed：'+
            }
            if(s.code === undefined) {
                if($.isPlainObject(s)) {
                    return callback(0,null, s);
                } else {
                    return callback(-102,null, r); // 'Server Response Not JSON 2：'+
                }
            } else if(s.code == 0) {
                return callback(0,s.data, s.message);
                //系统错误
            } else if(s.code < 0) {
                return callback(s.code,s.data, s.message);
                //业务逻辑错误
            } else {
                return callback(s.code,s.data, s.message);
        ...
        error: function(xhr, type) {
            if(retry > 1) {
                ...
                if((type != 'abort' && type != 'error') || xhr.status == 403 || xhr.status == 404) {
                    return callback(-1000,null, "xhr.responseText:"+xhr.responseText+', type:'+type);
                } else {
                    return callback(-1001,null, "xhr.responseText:"+xhr.responseText+', type:'+type);
...
$.xpost = function(url, postdata, callback, progress_callback) {
    ...
    $.ajax({
        ...
        success: function(r){
            if(!r) return callback(-1,null, 'Server Response Empty!');
            var s = xn.json_decode(r);
            if(!s || s.code === undefined) return callback(-1,null, 'Server Response Not JSON：'+r);
            if(s.code == 0) {
                return callback(0,s.data, s.message);
                //系统错误
            } else if(s.code < 0) {
        ...
        error: function(xhr, type) {
            ...
            if(type != 'abort' && type != 'error' || xhr.status == 403) {
                return callback(-1000,null,xhr.responseText);
            } else {
                return callback(-1001,null, xhr.responseText);
            }
        }
    });
};
